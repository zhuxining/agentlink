...
AgentLink 是一个**本地优先的 ACP-native Agent 编排层**。它复用用户已有的 ACP Server 执行能力，在本地管理 Agent 和 Skill，通过 Chat SDK 把 Agent 接入飞书等真实消息渠道。
...
5. **接入 IM 渠道**：启用飞书等渠道后，Agent 进入真实消息流。@提及开启新会话，内置命令切换 Agent，Agent 处理完直接回复到对话中。
