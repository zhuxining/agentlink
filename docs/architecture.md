...
AgentLink 的架构服务于产品蓝图（`docs/blueprint.md`），三条边界分层依赖——下层决定"能做什么"，上层决定"怎么触达"：

1. **ACP-native（底座）** — 不自研 Agent 执行运行时，所有 Agent 执行通过 ACP 协议调用用户配置的 ACP Server。AgentLink 管"谁处理、用什么 Skill、何时切换"，ACP Server 管"如何执行"。
2. **编排层（中间）** — 依赖 ACP 底座，在其之上管理 Agent、Skill、会话状态和内置命令。不绑定任何特定 ACP Server 的内部配置，编排逻辑与执行实现解耦。
3. **Messaging-native（上层）** — 通过 Chat SDK 把 Agent 接入真实 IM 消息流。IM 渠道是场景，不是平台。v1 优先实现飞书和 Telegram，后续扩展其他渠道。
...
| **AcpServer** | AgentLink（文件 `acp-servers.jsonc`） | ACP Server 连接配置——启动命令、参数。Agent 通过 `defaultAcpServer` 引用 |
...
```
feishu:channel_abc:thread_123        ← IM 线程（示例：飞书）
telegram:chat_xyz:thread_456         ← IM 线程（示例：Telegram）
...
│  Slack · Feishu · Telegram · Discord · GitHub ...       │
├─────────────────────────────────────────────┤
│           Agent Control Layer                │
│  Agent（*.md）· Skill（*.md）                 │
...
- **Channel Adapter Layer**：基于 Chat SDK 的 Adapter 体系。Chat SDK 提供 Slack、Feishu、Telegram、Discord、GitHub 等多平台适配器，处理 webhook 验签、消息解析、线程管理（`thread.subscribe()` / `thread.post()`）和事件分发（`onNewMention`、`onSubscribedMessage`、`onSlashCommand`、`onReaction` 等）。AgentLink 不重新实现协议适配逻辑。
...
| ACP Server 生命周期如何管理？ | 按需启动（首个 session），每次 `/use` fork 新 session 在同一进程，跨 Server 切换启动新进程；崩溃自动重启并报告错误 | 启动时全量启动所有 Server 或全局单例进程 | 按需启动减少空闲资源占用；同 Server 内复用进程降低启动延迟；崩溃不丢状态——session 在 ACP Server 侧持久化 |
...
Electron App
  ...
  │    │         └─ Handlers → 调用 Services 层执行业务逻辑
  │    ├─ Services 层
  │    │    ├─ 域逻辑封装（与 IPC 领域对齐）
  │    │    ├─ 内部状态管理
  │    │    └─ 外部依赖编排（Chat SDK、AcpClient 等）
  │    ├─ Chat SDK `Chat` 实例（多渠道适配器编排）
  │    │    ├─ Adapters（Slack / Feishu / Telegram / Discord / GitHub ...）
...
Services 层与 IPC 层并列，负责**业务逻辑**的封装和执行。IPC handlers 只做参数校验、权限检查和响应封装，实际业务逻辑委托给 services。

### 目录结构

```
src/services/
  ├─ agent/         — Agent 文件管理、AgentResolver 逻辑、Agent 索引构建
  ├─ channel/       — Chat SDK 适配器配置管理、启用/禁用
  ├─ conversation/  — Conversation 路由索引 CRUD、IM Thread ↔ ACP Session 映射
  ├─ workspace/     — Workspace 读写（workspaces.jsonc）、工作区切换
  ├─ acp/           — AcpServer 生命周期、AcpClient 调用、session 管理、permission 处理
  ├─ file-index/    — 目录扫描、文件解析、内存索引构建、chokidar 热更新
  └─ events/        — 内部 Events 订阅/发布管理
```

每个 service 领域遵循统一结构：

- `index.ts` — 导出分组（与 IPC 领域对齐，方便 handler 引用）
- `service.ts` — 业务逻辑实现
- `utils/`（可选）— 领域工具函数
...
| 层级 | 职责 | 禁止 |
|------|------|------|
| **IPC handlers** | 接收请求、Zod 校验、调用 service、组装响应 | 直接调用外部 SDK、AcpClient、Chat SDK API |
| **Services** | 执行业务逻辑、管理内部状态、编排外部依赖 | 直接访问 Renderer 进程、处理 HTTP 请求 |
| **外部依赖** | 提供底层能力（Chat SDK 消息通道、ACP 协议通信、SQLite 存储） | 包含业务规则 |

### 调用关系

```
Renderer Process
  └─ Actions（ipc.client.<domain>.<method>()）
       └─ IPC handlers（src/ipc/<domain>/handlers.ts）
            └─ Services（src/services/<domain>/service.ts）
                 ├─ Chat SDK（消息通道、线程管理）
                 ├─ AcpClient（协议通信、session 管理）
                 └─ SQLite（Conversation 路由、运行时状态）
```

**新增业务逻辑的步骤**：

1. 在 `src/services/<domain>/service.ts` 中实现业务逻辑
2. 在 `src/services/<domain>/index.ts` 中导出
3. 在 `src/ipc/<domain>/handlers.ts` 中通过 `os.handler()` 调用 service，做参数校验和响应封装
4. 在 `src/actions/<domain>.ts` 中创建 action 包装器供 React 组件使用
...
| `args` | string[] | 是 | 命令参数 |
| `description` | string | 否 | 一句话描述，Desktop UI 列表展示用 |
...
AgentLink 的测试遵循 AGENTS.md 约定：单元测试用 Vitest + jsdom + React Testing Library，E2E 用 Playwright（Electron + Chromium）。新增业务逻辑和关键路径必须包含测试。

### 测试分层

| 层 | 框架 | 覆盖目标 |
|----|------|----------|
| 单元测试 | Vitest + jsdom + RTL | Services 层纯逻辑：适配器注册/启用/禁用、Thread↔Session 映射、ACP 消息组装、AgentResolver 解析 |
| 集成测试 | Vitest（mock 外部依赖） | `AcpService` 以 mock ACP Server 子进程驱动 `session/new`→`prompt`→`textStream` 全流程 |
| E2E | Playwright | 渠道启用/禁用流程、对话列表与消息面板实时更新、ACP Server 配置 |

### 当前状态

Phase 1 核心链路（`src/services/chat`、`src/services/acp`）**暂无自动化测试**——`src/tests` 当前为空，是已知技术债。下一步补强优先级（详见 design spec § 验证）：

1. `adapter-registry.test.ts` — 注册、启用/禁用、状态机
2. `acp-session-mapper.test.ts` — Thread↔Session 映射与 SQLite 持久化
3. `acp-service.test.ts` — mock ACP 子进程跑通 prompt 流程
4. `chat-service.test.ts` — 事件处理器（`onNewMention`/`onSlashCommand`）路由到 Agent/ACP

## § 实现状态

本文档为架构设计的基线（`finalized`），描述的能力模型与设计决策是实现依据。当前实现进度以 `docs/superpowers/specs/2026-06-30-chat-sdk-acp-integration-design.md` 为准：

- **Phase 1（已实现）**：Channel Adapter Layer（飞书/Telegram）、ACP Integration Layer、Conversation 路由、实时事件推送、对应 Desktop UI。
- **Phase 2 / Phase 3（设计中）**：Agent/Skill 管理、Dashboard、斜杠命令、工作区——见 design spec，本文档不重复展开。

架构与实现的偏差（如有）以 design spec 的变更记录为准。
...
| 日期 | 变更 | 说明 |
|------|------|------|
| 2026-06-28 | 初稿 | 架构定位、三层能力模型、设计决策、IPC 模式、核心流程、数据模型、安全 |
| 2026-06-29 | 审核修订 | 架构定位改分层依赖表述；v1 渠道加 Telegram；核心概念补 AcpServer、修正 ACP 侧术语、加双渠道映射示例；决策表补 ACP Server 生命周期管理；主流程加按需启动步骤；acp-servers.jsonc 补 description 字段 |
| 2026-07-07 | 补写 | 新增 § 测试策略（含测试分层与 Phase 1 零测试现状）、§ 实现状态（Phase 1 已实现/Phase 2-3 设计中）；文档关联补 design spec 链接；与 blueprint.md 对齐 Phase 1 进度 |
